trigger EventsTrigger on Events__c (after insert, before update, before delete) {

    List<Events__c> existingRecords = new List<Events__c>();
    Map<Id, Integer> targetReviewCount = new Map<Id, Integer>();
    Map<Events__c, Integer> GoogleReviewCount= new Map<Events__c, Integer>();
    List<Id> GoogleReviewIdList = new List<Id>();

    if(Trigger.isInsert){
        List<Events__c> newRecords = Trigger.new;
        for (Events__c newGoogleReview : newRecords){
            GoogleReviewIdList.add(newGoogleReview.Id);
        }

        List<Events__c> existingRecords = [SELECT Target__c FROM Events__c WHERE Events__c.Id IN :GoogleReviewIdList];
        for (Events__c record : existingRecords) {
        ...
        for (Events__c record : newRecords) {
            if(GoogleReviewCount.containsKey(record)){
    ...
    if (Trigger.isDelete) {
        List<Events__c> deletedRecords = Trigger.old;
        for (Events__c oldGoogleReview : deletedRecords){
        ...
        for (Events__c record : deletedRecords) {
            if(GoogleReviewCount.containsKey(record)){
                Id targetId = record.Target__c;
                targetReviewCount.put(targetId, GoogleReviewCount.get(record));
            }
